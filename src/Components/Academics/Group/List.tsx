...
import { GET_GROUP_LIST, GET_ALL_GROUP, DELETE_GROUP_LIST } from "@/Urql/Query/Academics/group.query";
import { GetGroupListData, GetAllGroupData, DeleteGroupData } from "@/Urql/Types/Academics/group.types";
...
const List = () => {
    ...
    //Urql
    const [{ data, error, fetching }, refetch] = useQuery<GetGroupListData>({
        query: GET_GROUP_LIST,
        variables: { searchInput: variables },
        requestPolicy: policy
    });
    const [deleteData, deleteList] = useMutation<DeleteGroupData>(DELETE_GROUP_LIST);
    ...
    const onDeleteConfirm = (id: string) => {
        deleteList({ deleteGroupId: id }).then(() => {
    ...
    return (
        <div className="mb-8">
            {(deleteData.error || deleteData.data) &&
                ...
                >
                    {deleteData.error?.message ?? deleteData.data?.deleteGroup.message}
                </Notification>
            }
            <Header />
            <div className="mt-8 overflow-x-auto">
                <table className="table table-compact w-full">
                    ...
                    <tbody>
                        {data?.getGroups.results.map((item, i) => (
                            <tr key={i}>
                                <td>{item.name}</td>
                        ...
                        {data?.getGroups.results.length === 0 && !variables.search &&
                            <tr>
            ...
            <div className="mt-5">
                <Pagination
                    onNextClick={() => {
                        ...
                        setVariables?.(prev => ({ ...prev, page: data?.getGroups.meta.currentPage as number + 1 }))
                    }}
                    onPrevClick={() => {
                        setPolicy?.("network-only")
                        setVariables?.(prev => ({ ...prev, page: data?.getGroups.meta.currentPage as number - 1 }))
