...
const Header = () => {
    //Urql
    const [{ data }] = useQuery<GetAllGroupData>({ query: GET_ALL_GROUP });
    ...
    return (
        <div className="mt-8">
            <div className="flex gap-8 items-center">
                <p className="text-base uppercase font-semibold">Group List</p>
                ...
                <div className="flex-1 text-right">
                    <ExportTable
                        data={data?.getAllGroups.map((item) => ({ name: item.name, createdBy: item.createdBy.name || item.createdBy.phone })) as Array<any>}
                        headers={["Group Name", "Created By"]}
                        fileName="Group List"
                    />
                </div>
            </div>
