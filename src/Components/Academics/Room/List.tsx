...
import { GET_ROOM_LIST, DELETE_ROOM, GET_ALL_ROOM } from "@/Urql/Query/Academics/room.query";
import { GetRoomsListData, DeleteRoomsData, GetAllRoomData } from "@/Urql/Types/Academics/room.types";
...
const List = () => {
    ...
    //Urql
    const [{ data, error, fetching }, refetch] = useQuery<GetRoomsListData>({
        query: GET_ROOM_LIST,
        variables: { searchInput: variables },
        requestPolicy: policy
    });
    const [deleteData, deleteList] = useMutation<DeleteRoomsData>(DELETE_ROOM);
    ...
    const onDeleteConfirm = (id: string) => {
        deleteList({ deleteRoomId: id }).then(() => {
    ...
    return (
        <div className="mb-8">
            {(deleteData.error || deleteData.data) &&
                ...
                >
                    {deleteData.error?.message ?? deleteData.data?.deleteRoom.message}
                </Notification>
            }
            <Header />
            <div className="mt-8 overflow-x-auto">
                <table className="table table-compact w-full">
                    <thead>
                        <tr>
                            <th className="bg-primary capitalize text-main font-medium">Room No.</th>
                            <th className="bg-primary capitalize text-main font-medium">Capacity</th>
                    ...
                    <tbody>
                        {data?.getRooms.results.map((item, i) => (
                            <tr key={i}>
                                <td>{item.room_no}</td>
                                <td>{item.capacity}</td>
                        ...
                        {data?.getRooms.results.length === 0 && !variables.search &&
                            <tr>
                                <td colSpan={4} className="text-center text-main font-medium text-base py-4">
                                    No class room data found!
                                </td>
                            </tr>
                        }
                        {data?.getRooms.results.length === 0 && variables.search &&
            ...
            <div className="mt-5">
                <Pagination
                    onNextClick={() => {
                        ...
                        setVariables?.(prev => ({ ...prev, page: data?.getRooms.meta.currentPage as number + 1 }))
                    }}
                    onPrevClick={() => {
                        setPolicy?.("network-only")
                        setVariables?.(prev => ({ ...prev, page: data?.getRooms.meta.currentPage as number - 1 }))
