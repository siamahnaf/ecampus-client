...
const Header = () => {
    //Urql
    const [{ data }] = useQuery<GetAllShiftData>({ query: GET_ALL_SHIFT });
    ...
    return (
        <div className="mt-8">
            <div className="flex gap-8 items-center">
                <p className="text-base uppercase font-semibold">Shift List</p>
                ...
                <div className="flex-1 text-right">
                    <ExportTable
                        data={data?.getAllShifts.map((item) => ({ name: item.name, createdBy: item.createdBy.name || item.createdBy.phone })) as Array<any>}
                        headers={["Shift Name", "Created By"]}
                        fileName="Shift List"
                    />
                </div>
            </div>
