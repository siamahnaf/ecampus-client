...
import { GET_SHIFT_LIST, GET_ALL_SHIFT, DELETE_SHIFT } from "@/Urql/Query/Academics/shift.query";
import { GetShiftListData, GetAllShiftData, DeleteShiftData } from "@/Urql/Types/Academics/shift.types";
...
const List = () => {
    ...
    //Urql
    const [{ data, error, fetching }, refetch] = useQuery<GetShiftListData>({
        query: GET_SHIFT_LIST,
        variables: { searchInput: variables },
        requestPolicy: policy
    });
    const [deleteData, deleteList] = useMutation<DeleteShiftData>(DELETE_SHIFT);
    ...
    const onDeleteConfirm = (id: string) => {
        deleteList({ deleteShiftId: id }).then(() => {
    ...
    return (
        <div className="mb-8">
            {(deleteData.error || deleteData.data) &&
                ...
                >
                    {deleteData.error?.message ?? deleteData.data?.deleteShift.message}
                </Notification>
            }
            <Header />
            <div className="mt-8 overflow-x-auto">
                <table className="table table-compact w-full">
                    ...
                    <tbody>
                        {data?.getShifts.results.map((item, i) => (
                            <tr key={i}>
                                <td>{item.name}</td>
                        ...
                        {data?.getShifts.results.length === 0 && !variables.search &&
                            <tr>
            ...
            <div className="mt-5">
                <Pagination
                    onNextClick={() => {
                        ...
                        setVariables?.(prev => ({ ...prev, page: data?.getShifts.meta.currentPage as number + 1 }))
                    }}
                    onPrevClick={() => {
                        setPolicy?.("network-only")
                        setVariables?.(prev => ({ ...prev, page: data?.getShifts.meta.currentPage as number - 1 }))
