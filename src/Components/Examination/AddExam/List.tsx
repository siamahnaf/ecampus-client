...
import { GET_EXAM_LIST, GET_ALL_EXAM, DELETE_EXAM } from "@/Urql/Query/Examination/exam.query";
import { GetExamsData, GetAllExamsData, DeleteExamData } from "@/Urql/Types/Examination/exam.types";
...
const List = () => {
    ...
    //Urql
    const [{ data, error, fetching }, refetch] = useQuery<GetExamsData>({
        query: GET_EXAM_LIST,
        variables: { searchInput: variables },
        requestPolicy: policy
    });
    const [deleteData, deleteList] = useMutation<DeleteExamData>(DELETE_EXAM);
    ...
    const onDeleteConfirm = (id: string) => {
        deleteList({ deleteExamId: id }).then(() => {
    ...
    return (
        <div className="mb-8">
            {(deleteData.error || deleteData.data) &&
                ...
                >
                    {deleteData.error?.message ?? deleteData.data?.deleteExam.message}
                </Notification>
            }
            <Header />
            <div className="mt-8 overflow-x-auto">
                <table className="table table-compact w-full">
                    <thead>
                        <tr>
                            <th className="bg-primary capitalize text-main font-medium">Exam No.</th>
                            <th className="bg-primary capitalize text-main font-medium">Exam Name</th>
                            <th className="bg-primary capitalize text-main font-medium">Exam Type</th>
                    ...
                    <tbody>
                        {data?.getExams.results.map((item, i) => (
                            <tr key={i}>
                        ...
                        {data?.getExams.results.length === 0 && !variables.search &&
                            <tr>
            ...
            <div className="mt-5">
                <Pagination
                    onNextClick={() => {
                        ...
                        setVariables?.(prev => ({ ...prev, page: data?.getExams.meta.currentPage as number + 1 }))
                    }}
                    onPrevClick={() => {
                        setPolicy?.("network-only")
                        setVariables?.(prev => ({ ...prev, page: data?.getExams.meta.currentPage as number - 1 }))
