...
import { UPDATE_EXPENSE, GET_EXPENSE_LIST, GET_ALL_EXPENSE } from "@/Urql/Query/Expense/expense.query";
import { UpdateExpenseData, GetExpenseListData, GetAllExpense, ExpenseData } from "@/Urql/Types/Expense/expense.types";
...
const Edit = ({ open, onClose, defaultValue }: Props) => {
    ...
    //Urql
    const [{ data, error, fetching }, updateList] = useMutation<UpdateExpenseData>(UPDATE_EXPENSE);
    ...
    const onSubmit: SubmitHandler<Inputs> = (value) => {
        ...
        updateList({ expenseInput: expenseData, updateExpenseId: defaultValue.id }).then(({ data }) => {
    ...
    return (
        <div>
            {(error || data) &&
                ...
                >
                    {error?.message ?? data?.updateExpense.message}
                </Notification>
            }
            <Dialog
                open={open}
                                    ...
                                    render={({ field: { onChange, value } }) => {
                                        ...
                                        return (
                                            <Select
                                                ...
                                                error={errors.head ? true : false}
                                            >
                                                {headData.data?.getAllExpenseHead.map((item, i) => (
